...
It stores encrypted secrets (name, username, password, URL, comments) on the local machine and protects them with a master password.

Key features
------------
- Master-password protected: a user-provided master password is used to derive a symmetric key (Scrypt KDF) for encryption.
...
- CLI launcher: a small CLI entrypoint (`SimpleCredentialManagerCli.py`) that initializes the app and prompts for the master password.
- Cross-platform helper scripts: `SimpleCredentialManagerCli.bat` (Windows) and `SimpleCredentialManagerCli.sh` (POSIX) to prepare dependencies and launch the CLI.
...
Or use the provided Windows launcher `SimpleCredentialManagerCli.bat` which attempts to find Python and install requirements automatically.
